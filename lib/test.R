...
for(i in 1:testNum)
{
  ...
  filename <- sprintf("project4_data/data/%s/%s/%s/%s.h5",
                      substr(name[testset[i],],3,3),
                      substr(name[testset[i],],4,4),
                      substr(name[testset[i],],5,5),
                      name[testset[i],])
...
for(i in 1:testNum)
{
  ...
  r <- abs(rank(final.pre)-ncol(lyr))
  r.bar <- sum(r)/5000
  pre.ranksum[i] <- sum(r[which(lyr[testset[i],-1]>0)])/r.bar/sum(lyr[testset[i],-1]>0)
}
